...
echo -e "${BLUE}🦉 ULRATTACK Build Script${NC}"
echo "================================"
...
poetry run pyinstaller ulrattack.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="ulrattack-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/ulrattack.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/ulrattack.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/ulrattack" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/ulrattack" "$RELEASE_DIR/$BINARY_NAME"
