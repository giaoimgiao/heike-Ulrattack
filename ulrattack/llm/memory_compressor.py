...
class MemoryCompressor:
    def __init__(
        ...
        self.model_name = model_name or Config.get("ulrattack_llm")
        self.timeout = timeout or int(Config.get("ulrattack_memory_compressor_timeout") or "30")

        if not self.model_name:
            raise ValueError("ULRATTACK_LLM environment variable must be set and not empty")
