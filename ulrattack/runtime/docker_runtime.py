...
class DockerRuntime(AbstractRuntime):
        ...
        try:
            ...
            if isinstance(e, RequestsConnectionError | RequestsTimeout):
                raise SandboxInitializationError(
                    ...
                    "and try running ulrattack again.",
                ) from e
            raise SandboxInitializationError(
    ...
    def _create_container_with_retry(self, scan_id: str, max_retries: int = 3) -> Container:
        last_exception = None
        container_name = f"ulrattack-scan-{scan_id}"
        image_name = Config.get("ulrattack_image")
        if not image_name:
            raise ValueError("ULRATTACK_IMAGE must be configured")
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Check for port allocation errors (HTTP 500)
                is_port_error = "500" in str(e) and "port" in str(e).lower()
                if is_port_error:
                    logger.warning(f"Port conflict detected (HTTP 500), retrying with new ports...")
                ...
                # Wait longer for port errors to let system release resources
    ...
    def _get_or_create_scan_container(self, scan_id: str) -> Container:  # noqa: PLR0912
        ...
        try:
            ...
            if (
                "ulrattack-scan-id" not in container.labels
                or container.labels["ulrattack-scan-id"] != scan_id
            ):
                logger.warning(
        ...
        try:
            containers = self.client.containers.list(
                all=True, filters={"label": f"ulrattack-scan-id={scan_id}"}
            )
            if containers:
    ...
    def _initialize_container(
        ...
        execution_timeout = Config.get("ulrattack_sandbox_execution_timeout") or "120"
        ...
        self._wait_for_tool_server_health(health_url, container)
    ...
    def _wait_for_tool_server_health(
        ...
        try:
            log_result = container.exec_run("cat /tmp/tool_server.log", user="pentester")
            logs = log_result.output.decode(errors='replace')
        except Exception as e:
            logs = f"Failed to retrieve logs: {e}"

        error_msg = f"Tool server failed to start.\nContainer Logs:\n{logs}"
        logger.error(error_msg)

        raise SandboxInitializationError(
            "Tool server failed to start",
            f"{error_msg}\nPlease ensure Docker Desktop is installed and running, and try running ulrattack again.",
        )
